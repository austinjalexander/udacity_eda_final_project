...
library(dplyr)
by_age <- group_by(pf, age) %>%
  arrange(age)

pf.fc_by_age_gender <- by_age %>%
  #na.omit() %>%
  group_by(age, gender) %>%
...
cor(pf_subset$friendships_initiated, pf_subset$friend_count)
cor(pf_subset$age, pf_subset$mobile_likes)
```
...
ggplot(aes(x = tenure, y = prop_initiated),
       data = subset(pf, tenure >= 1 & !is.na(prop_initiated))) +
  geom_line(aes(color = year_joined.bucket),
            stat = 'summary',
            fun.y = mean)
...
largest <- subset(pf, year_joined >= 2012 & !is.na(prop_initiated))
mean(largest$prop_initiated)
